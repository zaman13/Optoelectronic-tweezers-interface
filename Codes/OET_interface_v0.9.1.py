...
def draw_window(active_ind):
    ...
    for m in range(len(px)):
        ...
        if p_type[m] >= 2:
            gear_original = pygame.transform.scale(sprites[p_type[m]-2], (s1[m], s1[m]))
            gear = gear_original.copy()
            gear_rect = gear_original.get_rect(center = (px[m],py[m]))
            
            sur_obj.blit(txt_label, gear_rect)

            rot[m] = rot[m] + (s3[m]-opening_radius_default)*step_rot*2


            gear = pygame.transform.rotate(gear, rot[m])
            gear_rect = gear.get_rect(center = gear_rect.center)
            
            # https://www.reddit.com/r/pygame/comments/2p1pi4/how_to_spin_an_image_on_its_center/
           

            sur_obj.blit(gear, gear_rect)
