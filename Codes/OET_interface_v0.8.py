...
gear_sprite = pygame.image.load('sprite2.svg').convert_alpha()        
print(gear_sprite)
...
def modify_object(active_ind, key_input):
    ...
    if key_input[pygame.K_d]:
        s3[active_ind] = s3[active_ind] - step_s if s3[active_ind] > 0 else 0  
...
def change_active_object(event,active_ind):
    ...
    if event.key ==  pygame.K_u:
        rot[active_ind] = np.pi/5 if p_type[active_ind] == 2 else rot[active_ind]
...
def draw_window(active_ind):
    ...
    for m in range(len(px)):
        ...
        img = font.render(label_str, True,
        ...
        if p_type[m] == 1:
            ...
            sur_obj.blit(img, (px[m]+0.9*s1[m], py[m]-16))
            
                    
        if p_type[m] == 2:
            gear = pygame.transform.scale(gear_sprite, (s1[m], s1[m]))
            rot[m] = rot[m] + np.pi/5
            gear = pygame.transform.rotate(gear, rot[m])
            sur_obj.blit(gear, (px[m], py[m]))
            sur_obj.blit(img, (px[m]+0.9*s1[m], py[m]-16))
