...
s         : Decrease width of the object
...
RALT + n  : Create line sprite (45 degree) (right alt key only)
...
o         : Create opening
r         : Rotate opening counter-clockwise (CCW)
f         : Rotate opening clockwise (CW)
e         : Increase radius of the opening. For sprites, this would create/increase speed of continuous rotation (CCW).
d         : Decrease radius of the opening. For sprites, this would create/increase speed of continuous rotation (CW). 
...
CTRL + t  : Include active object in a group
ALT + t   : Remove active object from group
...
Version updates:
    ...
    0.9.4: Feb 10, 2022: Save data file using current data/time string. Use rectangle instead of circle to create opening.
    0.9.5: Nov  3, 2023: Added grouping feature. Multiple objects can be grouped together and moved simultaneously. Besides movement, other properties of the objects are not grouped.
...
read_data_from_file = 'n'               # read data from file? y/n
data_file_name = 'sorter.csv'           # name of the data file to read
...
def move_object(active_ind, key_input):
    ...
    if active_ind not in group_ind_set:   # if active objecct is not in the group, then move it as usual
    ...
    else: # if active objecct is in the group, then move it all objects of the group together
       
                
        for ind in group_ind_set:   # for all indices in the group
            if key_input[pygame.K_LEFT]:
                px[ind] -= step
            if key_input[pygame.K_UP]:
                py[ind] -= step
...
def change_active_object(event,active_ind):
    ...
    # Modify group (add or remove)     (Nov. 3, 2023)
    if event.key == pygame.K_t:
        mods = pygame.key.get_mods()
        # Add active object to group
        if mods & pygame.KMOD_CTRL: 
            if active_ind not in group_ind_set:  # if active object is not already in the group
                group_ind_set.append(active_ind) 
        # Remove active object from group
        if mods & pygame.KMOD_ALT:   
            if active_ind in group_ind_set:      # if the active object is in the group
                group_ind_set.remove(active_ind) 
...
def open_circle(active_ind):
    ...
    # tx = px[active_ind]+s1[active_ind] - s2[active_ind]  # x coordinate of the opening is set at the middle of the right rim along x axis
    ...
    # cntr = [px[active_ind]+s1[active_ind]-1.1*s2[active_ind],py[active_ind]-0.5*s3[active_ind],1.2*s2[active_ind],s3[active_ind]] 
    
    p1x = px[active_ind] + s1[active_ind] - s2[active_ind] - 10
    p1y = py[active_ind] - s3[active_ind]*0.5
    p2x = px[active_ind] + s1[active_ind] + s2[active_ind] - 4
    p2y = py[active_ind] - s3[active_ind]*0.5
    p3x = px[active_ind] + s1[active_ind] + s2[active_ind] - 4
    p3y = py[active_ind] + s3[active_ind]*0.5
    p4x = px[active_ind] + s1[active_ind] - s2[active_ind] - 10
    p4y = py[active_ind] + s3[active_ind]*0.5
    
    
    [p1x,p1y] = rotate(p1x,p1y,px[active_ind],py[active_ind],rot[active_ind])
    [p2x,p2y] = rotate(p2x,p2y,px[active_ind],py[active_ind],rot[active_ind])
    [p3x,p3y] = rotate(p3x,p3y,px[active_ind],py[active_ind],rot[active_ind])
    [p4x,p4y] = rotate(p4x,p4y,px[active_ind],py[active_ind],rot[active_ind])
    
    
    
    
    cntr = [(p1x,p1y),(p2x,p2y),(p3x,p3y),(p4x,p4y)]
    
    # pygame.draw.circle(sur_obj, White, (px[active_ind], py[active_ind]),s3[active_ind],0, draw_top_right = True)
    # pygame.draw.circle(sur_obj, background_color, (tx, ty),s3[active_ind],0)
    pygame.draw.polygon(sur_obj, background_color, cntr)
    # black_box = pygame.image.rect(sur_obj, background_color, (tx,ty, s3[active_ind], s3[active_ind]))
    # sur_obj.blit(black_box)
    
    # pygame.draw.arc(sur_obj, White, [tx, ty,s2[active_ind], s2[active_ind]],0+rot[active_ind],np.pi/2 + rot[active_ind],10)
    return 0
#==============================================================================
...
def draw_window(active_ind):
    ...
    for m in range(len(px)):
        ...
        # Create text label. The active object will have the symbol "#" in its label
        ...
        txt_label = font.render(label_str, True,
                  ...
                  pygame.Color(background_color))
        txt_label.set_alpha(100)       # set transparency value (0-255)
...
#==============================================================================
# Function for writing data file
#==============================================================================

def data_write():
    date_time = datetime.now()
    yr = date_time.strftime("%Y_")
    mn = date_time.strftime("%m_")
    dy = date_time.strftime("%d_")
    tm = date_time.strftime("%H-%M-%S.csv")

    fsave_string = yr+mn+dy+tm
    
    
    with open(fsave_string, 'w', newline='') as csvfile:
...
active_ind = 0
group_ind_set = []    # define a list that contains indices of all grouped objects
